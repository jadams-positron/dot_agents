...
Before any upstream push or PR creation, perform a thorough multi-angle review of the complete local diff using the agent-pr-review methodology, but do not post a GitHub review. Its mandatory abstraction-review leg must satisfy `abstraction-review/references/independent-dispatch.md` (`independent-abstraction-review/v1`) with the `diff` profile. Freeze the target ref, full target-tip, merge-base, head and tree OIDs, exact binary diff and SHA-256; give the fresh reviewer only issue intent, repository instructions, and that frozen target. Stop if the canonical context or capability boundary cannot be enforced. Retain the complete evidence packet, including the deterministic pre-use observation. This leg may satisfy work-issue's abstraction gate for the same exact target. Run fix-all on every validated finding, rerun all relevant tests, and repeat with another fresh abstraction reviewer until clean. Then continue the work-issue workflow.
