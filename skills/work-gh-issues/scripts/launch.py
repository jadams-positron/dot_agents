...
Before any upstream push or PR creation, perform a thorough multi-angle review of the complete local diff using the agent-pr-review methodology, but do not post a GitHub review. Require a clean worktree, freeze edits, and record full 40-hex target-tip, merge-base, and head commit OIDs plus the exact diff digest before dispatch. Include the methodology's mandatory abstraction-review leg in a fresh read-only agent with no inherited conversation or reviewer context; give that agent only the issue intent, repository instructions, immutable target-tip/merge-base/head OIDs, and raw diff location. Enforce a positive tool allowlist containing only read/search and exact non-mutating history/diff commands; deny mutation-capable tools and stop if enforcement is unavailable. Refetch and verify all three OIDs, the digest, and worktree cleanliness after collection; discard and restart the review on any mismatch. This leg may satisfy work-issue's abstraction gate for the same exact diff. Run fix-all on every validated finding, rerun all relevant tests, and repeat with another fresh abstraction reviewer until clean. Then continue the work-issue workflow.
...
After all branches pass their local gates, run `gh stack rebase` before freezing the aggregate review target, then rerun every affected local gate. Require a clean worktree, freeze edits, fetch the default base, and record its full 40-hex tip OID, the full merge-base and stack-tip OIDs, and the exact diff SHA-256. Run the local multi-angle review and fix-all workflow across that exact complete stack. Include the agent-pr-review methodology's mandatory abstraction-review leg in a fresh read-only agent with no inherited conversation or reviewer context. Give it only the raw title/body and acceptance criteria for every stack issue, repository instructions, those immutable endpoints, and the diff artifact—never implementation plans or prior findings. Enforce a positive tool allowlist containing only read/search and exact non-mutating history/diff commands; deny mutation-capable tools and stop if enforcement is unavailable. Re-resolve and verify all OIDs, the digest, and worktree cleanliness after collection; discard and restart on any mismatch. Repeat with another fresh abstraction reviewer after repairs until clean. Use pr-description as the sole PR-body authoring path to write and validate a separate body file for each future PR from only its immediate base-to-head diff; never reuse a cumulative body. No amend, rebase, sync, or content-changing command may occur between the final review snapshot and `gh stack submit --auto`; if one is required, discard the review and bodies, perform it, reverify, and rerun with a different fresh abstraction reviewer. Submit without another rebase, apply the prepared bodies, and correct every PR's title, assignee, labels, `Closes #<issue>`, and immediate base. Record the ordered branches, PRs, bases, worktree, owner, and expected remote SHAs.

Handle CI and Bugbot bottom-to-tip. Amend fixes into the owning issue commit, run `gh stack rebase --upstack`, and use `gh stack sync` to atomically update the chain. Every content amendment or cascade rebase invalidates the aggregate abstraction review. After the stack stabilizes and before any PR becomes ready, require a clean worktree, freeze the new full default-base tip, merge-base, stack-tip OIDs, and diff digest; rerun the review with a different fresh abstraction-review agent against only the raw issue intent, repository instructions, and those immutable inputs, and verify them plus worktree cleanliness again after collection. Apply fix-all and repeat the stack rebase/sync, checks, and fresh review until the final tip is clean. Recheck every affected descendant on its new SHA. Refresh each affected PR through pr-description against its final immediate base and head. Before rewriting a live body, preserve any Bugbot summary appended at the end byte-for-byte and validate against the live snapshot with --existing-body. Never repair or push one parent branch in isolation, never merge, and never add AI attribution."""
    return append_extra_instructions(prompt, extra_instructions)
